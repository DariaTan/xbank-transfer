"""Full-scale MLM inference: for every client, one embedding per calendar
...
The embedding is the last-real-event hidden state from an unmasked
bidirectional encoding of that window (`MLM.encode` + `last_event_
embedding`, see models/mlm.py) -- safe from future leakage only
because the window itself never extends past the target date (see that
module's docstring caveat).

Reuses the trained checkpoint's OWN preprocessor (`preprocessor.pkl`,
saved by train_mlm.py) -- see infer_coles.py's identical rationale.
`max_position_embeddings` is read directly off the checkpoint's own
`backbone.embeddings.position_embeddings.weight` shape rather than
hardcoded/recomputed from `max_seq_len`: MLM's BERT backbone has a
real learned position-embedding table (unlike NEP's RoPE, which has
none), so a checkpoint trained under an older max_position_embeddings
convention (e.g. `max_seq_len + 8`, before that was simplified to just
`max_seq_len`) must be rebuilt with ITS OWN value or `load_state_dict`
shape-mismatches on that table.

Run parameters live in configs/models/downstream.yaml's `inference:`
section (shared across all infer_*.py scripts) and configs/models/mlm.yaml
...
Output: one parquet file per target date under <embeds_dir>/mlm/, columns
...
def main():
    ...
    parser.add_argument("--model-config", type=str, default="/app/configs/models/mlm.yaml")
    ...
    out_dir = Path(inf["embeds_dir"]) / "mlm"
    ...
    print(f"Loading MLM checkpoint from {ckpt_dir / 'best.pt'} ...", flush=True)
    ...
    model.to(device)
    print(f"  loaded (max_position_embeddings={ckpt_max_pos}).", flush=True)
