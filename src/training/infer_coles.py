...
configs/models/coles.yaml (this architecture's own hidden_size/
embedding_dim/num_layers/checkpoint_dir -- must match what the checkpoint
was actually trained with). The only CLI flags are --downstream-config/
--model-config, to point at different files.

Output: one parquet file per target date under <embeds_dir>/coles/,
columns [inn, date, emb_0..emb_<hidden_size-1>] -- resumable the same way
infer_chronos2.py's chunking is: a date whose file already exists is
...
def main():
    ...
    out_dir = Path(inf["embeds_dir"]) / "coles"
