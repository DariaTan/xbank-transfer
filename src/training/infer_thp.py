...
(this architecture's own hidden_size/num_layers/checkpoint_dir -- must
match what the checkpoint was actually trained with). The only CLI flags
are --downstream-config/--model-config, to point at different files.

Output: one parquet file per target date under <embeds_dir>/thp/, columns
[inn, date, emb_0..emb_<hidden_size-1>] -- resumable, a date whose file
...
def main():
    ...
    out_dir = Path(inf["embeds_dir"]) / "thp"
