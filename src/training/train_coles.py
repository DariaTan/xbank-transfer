...
def main():
    ...
    if args.n_clients is not None:
        ...
        # -- that's what silently OOM-killed train_nep.py against MBD's
        # ~550M-row daily table (2026-09-14). Same design as train_nep.py's
        # identical chunked branch: a client's rows never split across
        # chunks, train/valid membership is decided ONCE upfront on the
        # cheap id list, and the category vocabulary is fit on the FIRST
        # chunk containing train clients (chunks are pre-shuffled, so
        # this is a random ~100K-client sample) and reused (transform-only)
