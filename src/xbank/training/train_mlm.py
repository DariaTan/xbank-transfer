...
validation loss, resumable checkpointing. Mirrors scripts/train_nep.py.

Usage (inside the container; pin a GPU via CUDA_VISIBLE_DEVICES on the
`docker exec` call, see TRAINING.md):
    docker exec -e CUDA_VISIBLE_DEVICES=1 xbank-transfer python scripts/train_mlm.py
"""
import argparse
...
from xbank.models.mlm import MLM
...
def main():
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="outputs/checkpoints/mlm")
